...
function GeneratingView({ progress }: { progress?: JobProgress }) {
  const [fallbackPct, setFallbackPct] = useState(8);
  const pct = progress?.progress ?? fallbackPct;
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      setFallbackPct((prev) => Math.min(prev + 3, 92));
    }, 1200);
...
function PaperView({
  ...
  return (
    <div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className="version-label">Generated </span>v{paper.version}
          </span>
          <span style={{ fontSize: 13, color: "var(--slate-light)" }}>
            {paper.sections.reduce((s, sec) => s + sec.questions.length, 0)}{" "}
            questions · {paper.maximumMarks} marks
          </span>
        </div>
        ...
        >
          <RefreshCw
            size={15}
            style={isRegen ? { animation: "spin 1s linear infinite" } : {}}
          />
          Regenerate
        </button>
      ...
      {/* Student info strip */}
      <div
        className="no-print output-student-strip"
        style={{
      ...
      {/* ── The Exam Paper ── */}
      <div
        className="output-paper-wrap"
        style={{ padding: "32px 32px 64px", background: "var(--surface)" }}
      >
        <div
          ref={paperRef}
        ...
        >
          ...
          >
            ...
            {/* Student info */}
            <div
              style={{ display: "flex", gap: 32, marginTop: 16, fontSize: 12 }}
            >
              <span>
                Name:{" "}
                <span
                            ...
                            return (
                              ...
                              >
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div
                                    className="output-question-meta"
                                    style={{
                                      display: "flex",
                                      justifyContent: "space-between",
                                      alignItems: "flex-start",
                              ...
                              {question.text}
                                    ...
                                    {question.imageGenerationStatus ===
                                      "pending" && (
                                      <div
                                        style={{
                                          fontSize: 11,
...
export default function OutputPage() {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(async () => {
      try {
        ...
        console.error("Polling assignment status failed:", err);
      }
    }, 4000);
  ...
  const isGenerating =
    currentAssignment?.status === "generating" || progress?.status === "active";
        ...
        currentAssignment?.status === "failed" && (
          ...
          >
            ...
            <p style={{ color: "var(--slate-light)" }}>
              The AI generation failed. Please try regenerating.
            </p>
            <button
